...
//  PARTYGUARD
//
//  Created by Project NMWSU on 11/27/16.
//  Copyright © 2016 High Fliers. All rights reserved.
//
...
public struct ActivitySpinner {
    ...
    /// Disable UI stops users touch actions until Activity is hidden. Return success status
    @discardableResult
    ...
    public static func hide(_ success: Bool? = nil, animated: Bool = false) -> Bool {
        guard instance != nil else {
            print("Activity: You don't have an activity instance")
            return false
        }
        
        guard hidingInProgress == false else {
            print("Activity: Hiding already in progress")
            return false
        }
